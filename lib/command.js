...
var commandArgs = require('optimist').argv
...
var pipeNodeStreamThroughHandler = function(handler, args, nodeReadStream, nodeWriteStream, callback) {
  ...
  handler(commandArgs, inputStreamable, function(err, resultStreamable) {
    if(err) return callback(err)
...
var doCommand = function(commandArgs, process, callback) {
...
doCommand(commandArgs, process, function(err) {
  if(err) throw err
})